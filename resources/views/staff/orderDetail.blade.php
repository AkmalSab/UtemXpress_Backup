        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <hr>
                    ...
                    Receiver Telephone: {{$item->receiver_phone}}<b></b>
                <br>
                {{-- runner section --}}
                ...
                @if($item->order_status == 'completed' && $item->user_rating == '' && $item->user_review == '')
                    ...
                    <form method="POST" action="{{url('/staff/rateOrder/'.$item->order_id)}}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="runnerReview" name="runnerReview" rows="3" required></textarea>
                        </div>
                ...
                @endif
                {{-- button section --}}
                <a href="{{url('/staff/activeOrder')}}" class="btn btn-primary">Back</a>
                ...
                @if($item->order_status == 'on-going' || $item->order_status == 'waiting')
                    <a href="{{url('/staff/cancelOrder/'.$item->order_id)}}" class="btn btn-danger">Cancel Order</a>
                @elseif($item->order_status == 'completed' && $item->favourite == '')
                    @if($isRunnerAlreadyFavourite == 0)
                        <a href="{{url('/staff/addToFavourite/'.$item->runner_id.'/'.$item->order_id)}}" class="btn btn-success">Add Runner to Favourite</a>
                    @endif
                @endif
            </div>
